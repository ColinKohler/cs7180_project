...
class RNMN(nn.Module):
  def __init__(self, query_size, hidden_size, device, mt_norm = 1, comp_length = 5, comp_stop_type = 1):
    ...
    self.attention_modules = [Id(), And(), Or(), self.find, self.relocate]
    ...
    self.x_size = 256
    max_len = 7
    self.decoder = Decoder(max_len, self.hidden_size, self.M_size, self.x_size, self.device, 1, mt_norm)
  ...
  def forward(self, query, query_len, context, debug=False):
    ...
    for t in range(self.comp_length):
      #TODO: Choose in some way
      init_out = torch.randn((1, batch_size, self.decoder.input_dim), requires_grad=True, device=self.device)
      self.M_t, self.x_t, stop_bits = self.decoder(init_out, encoded_query, debug=debug)
    ...
    for i, module in enumerate(self.attention_modules + self.answer_modules):
      ...
      if type(module) is Id:
        ...
        out = module.forward(attention)
          ...
          #exist_input_total = torch.cumsum(attention.view(batch_size,-1),dim=1)[:,-1]
          #print(exist_input_total[0].item())
      else:
